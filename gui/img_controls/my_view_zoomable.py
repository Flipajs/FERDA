...
class MyViewZoomable(QtGui.QGraphicsView, object):
    ...
    def keyPressEvent(self, event):
        ...
        if event_key == key_o or event_key == key_i:
            dir = "in" if event_key == key_i else "out"
            double_press = True if time.time() - self.time_since < TIMEOUT else False
            if double_press:
                # do_it = True
                # while do_it:
                #     print("prib")
                #     # m11, m22 = self.transform().m11(), self.transform().m22()
                #     # print(str(m11) + " " + str(m22))
                #     self.zoom(dir)
                #     do_it = True
                #     print(str(self.transform().m11()))
                self.zoom(dir)
            else:
                self.zoom(dir)
    ...
    def zoom(self, dir):
        ...
        if dir == "in" and (m11 < 2 or m22 < 2):
            time_line.valueChanged.connect(self.scale_in)
        elif dir == "out" and (m11 > 0.1 or m22 > 0.1):
    ...
    #     rect = self.ngv.scene.itemsBoundingRect()
    #     height, width = rect.height(), rect.width()
    #     if self.ngv.show_vertically:
    #         height, width = 5, rect.width()
    #     else:
    #         height, width = rect.height(), 5
    ...
    #     key_b = QtCore.Qt.Key_B
    #     self.node_1 = self.ngv.selected_edge[0][0]
    #     self.node_2 = self.ngv.selected_edge[0][1]
    #     if event_key == key_b:
    ...
    #         self.position_from = self.ngv.selected_edge[1]
