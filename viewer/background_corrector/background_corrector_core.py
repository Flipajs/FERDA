...
class BackgroundCorrector(QtGui.QMainWindow):
    ...
    def __init__(self, img, finish_callback):
        super(BackgroundCorrector, self).__init__()
        ...
        self.resize(800, 600)
        self.setWindowTitle("Background Corrector")
        ...
        self.open_action = QtGui.QAction("Open image", self.centralWidget())
        self.save_action = QtGui.QAction("Save image", self.centralWidget())
        self.fix_action = QtGui.QAction("Fix image", self.centralWidget())
        self.settings_action = QtGui.QAction("Settings", self.centralWidget())
        self.restore_action = QtGui.QAction("Reset image", self.centralWidget())
        self.finish_action = QtGui.QAction("Finish", self.centralWidget())
        self.cancel_action = QtGui.QAction("Cancel fixing", self.centralWidget())
        self.undo_action = QtGui.QAction("Undo", self.centralWidget())
        ...
        self.img_original = copy.deepcopy(img)
    ...
    def load_image(self, filename):
        ...
        arr = numpy.asarray(ptr).reshape((img.height(), img.width(), 3))
