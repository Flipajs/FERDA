...
def comparison2latex_tuned(name1='default', name2='', out_name='', highlight=True):
    from pylatex import Document, Section, Subsection, Tabular, Tabularx, MultiColumn, MultiRow
    from pylatex.utils import bold, italic, verbatim, escape_latex, NoEscape
    from pylatex.package import Package
...
if __name__ == '__main__':
    ...
    # comparison2latex(name2='_tuned', out_name='_tuned')
    comparison2latex_tuned(name1='_tuned', name2=None, out_name='_tuned2')
    # comparison2latex_tuned
    # #
    #
    ...
    if False:
        ...
        for pname in ['Cam1', 'Zebr', 'Came', 'Sowb']:
            ...
            for i in range(len(resultsc)):
                ...
                print("{:.2%}".format(old_div(np.sum(x), float(r1.shape[0]))))
                print(np.sum(x), r1.shape[0], resultsc[i][1], resultsi[i][1])

                vals.append(old_div(np.sum(x), float(r1.shape[0])))
