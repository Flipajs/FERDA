...
class EuclideanProjectiveTestCase(unittest.TestCase):
    def test_p2e(self):
        ...
        assert_array_equal(core.region.ep.p2e(np.array([2, 2, 1])), np.array([2, 2]))
        assert_array_equal(core.region.ep.p2e(np.array([2, 2, 2])), np.array([1, 1]))

    def test_e2p(self):
        euclid_2d = np.array([[1., 2.]]).T
        proj_2d = core.region.ep.e2p(euclid_2d)
        self.assertEqual(proj_2d.shape, (3, 1))
        assert_array_equal(proj_2d, np.vstack((euclid_2d, 1)))

        euclid_3d = np.array([[1., 2., 3.]]).T
        proj_3d = core.region.ep.e2p(euclid_3d)
        self.assertEqual(proj_3d.shape, (4, 1))
        assert_array_equal(proj_3d, np.vstack((euclid_3d, 1)))

        assert_array_equal(core.region.ep.e2p(np.array([2, 2])), np.array([2, 2, 1]))
