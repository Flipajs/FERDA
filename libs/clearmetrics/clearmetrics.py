    ...
    def _get_sq_distance_matrix(self, frame):
        ...
        for i in xrange(n_gt):
            ...
            for j in xrange(n_meas):
                ...
                is_list = type(gt_pos) is list
                is_none_inside_list = is_list and (None in gt_pos or None in measured_pos)

                if is_none_inside_list or gt_pos is None or measured_pos is None:
                    distance_mat[i, j] = sys.maxint
                else:
                    distance_mat[i, j] = np.sum(np.subtract(measured_pos, gt_pos) ** 2)
        return distance_mat
